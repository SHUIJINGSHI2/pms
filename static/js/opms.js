$(function(){
	...
	*/
	...
	$('#machine-more-form').validate({
		ignore:'',
		rules : {
			'addText':{required:true}
		},
		messages : {
			'addText':{required:'机器列表不能为空'}
		},
		submitHandler:function(form) {
			$(form).ajaxSubmit({
				type:'POST',
				...
				success:function(data) {
					dialogInfo(data.message)
					if (data.code) {
						setTimeout(function(){ window.location.href='/machine/manage'; }, 2000);
					} else {
						setTimeout(function(){ $('#dialogInfo').modal('hide'); }, 1000);
					}
				}
	...
	$('#machine-form').validate({
		...
		messages : {
			'machineid':{required:'机器ID 不能为空'},
			'business':{required:'业务用途不能为空'},
			'password':{required:'机器密码不能为空'}
		},
		submitHandler:function(form) {
			$(form).ajaxSubmit({
				type:'POST',
	...
	$('.js-machine-delete').on('click', function(){
		var that = $(this);
		var leaveid = that.attr('data-id');
		$.post('/machine/ajax/delete', {id:leaveid},function(data){
			dialogInfo(data.message)
			if (data.code) {
				setTimeout(function(){ window.location.reload(); }, 2000);
			} else {
...
function dialogInfo(msg) {
    ...
    html += '<h4 class="modal-title" id="dialogInfoTitle">AMS提示</h4>';
    html += ' </div>';
